...
function checkUser(str){
	if(str==""){			//当用户名为空时
		document.getElementById("username-error").innerHTML="请输入用户名！";//设置提示文字
		//document.getElementById("tr_user").style.display='block';	//显示提示信息
		flag_user=false;
	}else if(!checkeUser(str)){	//判断用户名是否符合要求
		document.getElementById("username-error").innerHTML="您输入的用户名不合法！长度限制为20个字母或10个汉字";	//设置提示文字
		//document.getElementById("tr_user").style.display='block';		//显示提示信息
		flag_user=false;
...
function deal(){
	...
	if(result=="1"){											//当用户名没有被注册
		document.getElementById("username-error").innerHTML="";		//清空提示文字
		//document.getElementById("tr_user").style.display='none';		//隐藏提示信息显示行
		flag_user=true;		
	}else{												//当用户名已经被注册
		document.getElementById("username-error").innerHTML=result;		//设置提示文字
		//document.getElementById("tr_user").style.display='block';		//显示提示信息
		flag_user=false;
	}
...
function checkPwd(str){
	if(str==""){		//当密码为空时
		document.getElementById("password-error").innerHTML="请输入密码！";	//设置提示文字
		//document.getElementById("tr_pwd").style.display='block';		//显示提示信息
		flag_pwd=false;
	}else if(!checkePwd(str)){		//当密码不合法时
		document.getElementById("password-error").innerHTML="您输入的密码不合法！密码由字母开头的字母、数字或下划线组成，并且密码的长度大于6位小于30位";	//设置提示文字
		//document.getElementById("tr_pwd").style.display='block';	//显示提示信息
		flag_pwd=false;
	}else{		//当密码合法时
		document.getElementById("password-error").innerHTML="";	//清空提示文字
...
function checkRepwd(str){
	if(str==""){		//当确认密码为空时
		document.getElementById("repassword-error").innerHTML="请确认密码！";	//设置提示文字
		//document.getElementById("tr_pwd").style.display='block';	//显示提示信息
		flag_repwd=false;
	}else if(form1.pwd.value!=str){		//当确认密码与输入的密码不一致时
		document.getElementById("repassword-error").innerHTML="两次输入的密码不一致！";	//设置提示文字
		//document.getElementById("tr_pwd").style.display='block';	//显示提示信息
		flag_repwd=false;
	}else{	//当两次输入的密码一致时
		document.getElementById("repassword-error").innerHTML="";	//清空提示文字
		//document.getElementById("tr_pwd").style.display='none';		//隐藏提示信息显示行
		flag_repwd=true;
	}
}
...
function checkEmail(str){
	if(str==""){//当E-mail地址为空时
		document.getElementById("email-error").innerHTML="请输入E-mail地址！";//设置提示信息
		//document.getElementById("tr_email").style.display='block';		//显示提示信息
		flag_email=false;
	}else if(!checkemail(str)){//当E-mail地址不合法时
		document.getElementById("email-error").innerHTML="您输入的E-mail地址不正确！";//设置提示信息
		//document.getElementById("tr_email").style.display='block';		//显示提示信息
		flag_email=false;
	}else{
		document.getElementById("email-error").innerHTML="";//清空提示信息
...
<div id="register" class="dataEye" style="z-index:11;display:none;margin-top:30px">
	<div class="loginbox registbox">
		<div class="login-content reg-content">
		    <div class="loginbox-title" style="float:right;">
		        <h3 onClick="Myclose('register')">关闭</h3>
		    </div>
			<div class="loginbox-title">
				<h3>注册</h3>
			</div>
			<form id="signupForm" name="form1" method="post" action="">
			<div class="error" id="username-error" style="margin-left: 10px"></div>
		    <div class="row">
				<label class="field" for="user">用户名</label>
				<input type="text" value="" class="input-text-user noPic input-click" name="user" id="user" onBlur="checkUser(this.value)">
			</div>
			<div class="error" id="email-error" style="margin-left: 10px"></div>
			<div class="row">
				<label class="field" for="email">注册邮箱</label>
				<input type="text" value="" class="input-text-user noPic input-click" name="email" id="email" onBlur="checkEmail(this.value)">
			</div>
			<div class="error" id="password-error" style="margin-left: 10px"></div>
			<div class="row">
				<label class="field" for="pwd">密码</label>
				<input type="password" value="" class="input-text-password noPic input-click" name="pwd" id="pwd" onBlur="checkPwd(this.value)">
			</div>
			<div class="error" id="repassword-error" style="margin-left: 10px"></div>
			<div class="row">
				<label class="field" for="repwd">确认密码</label>
				<input type="password" value="" class="input-text-password noPic input-click" name="repwd" id="repwd" onBlur="checkRepwd(this.value)">
			</div>
			<!-- <div class="row tips">
			...
			<div class="row btnArea">
				<a class="login-btn" id="submit" onClick="save()">注册</a>
			</div>
			</form>
		</div>
		<!-- <div class="go-regist">
