...
public class CreateImg extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		if ("Ocean".equals(template)) {
			...
			outWord(g, content, weather, 45, 165);
		} else if ("Bear".equals(template)) {
	...
	public void outWord(Graphics g, String[] content, String weather,
		...
		Font mFont = new Font("微软雅黑", Font.PLAIN, 15); // 通过Font构造字体
...
//		g.setFont(mFont); // 设置字体
		...
		String newTime = new SimpleDateFormat("yyyy年M月d日 E")
				.format(date);
		g.drawString(newTime + " " + weather, 15, 30);
...
//			x = 45 + (i % 3) * 195 + offsetX;
//			y = 130 + (i / 3) * 170 + offsetY;
			x = (i % 3) * 195 + offsetX;
			y = (i / 3) * 170 + offsetY;
...
//				} // 构造Image对象
			...
			if (contentLen < 5) {
				switch (contentLen % 5) {
				case 1:
					mFont = new Font("楷体", Font.PLAIN, 40); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 40, y);
					break;
				case 2:
					mFont = new Font("楷体", Font.PLAIN, 36); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 25, y);
					break;
				case 3:
					mFont = new Font("楷体", Font.PLAIN, 30); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 20, y);
					break;
				case 4:
					mFont = new Font("楷体", Font.PLAIN, 28); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 5, y);
				}
			} else {
				mFont = new Font("楷体", Font.PLAIN, 22); // 通过Font构造字体
				g.setFont(mFont); // 设置字体
				...
				} else if (Math.ceil(contentLen / 5.0) == 4) {
					//分四行写
					g.drawString(content[i].substring(0, 5), x, y - 30);
					g.drawString(content[i].substring(5, 10), x, y - 5);
					g.drawString(content[i].substring(10, 15), x, y + 20);
					g.drawString(content[i].substring(15), x , y + 45);
				}
			}
		}
		g.dispose();
