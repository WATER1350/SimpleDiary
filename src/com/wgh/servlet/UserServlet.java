...
public class UserServlet extends HttpServlet {
	...
	private void login(HttpServletRequest request, HttpServletResponse response)
		...
		log.info(request.getParameter("pwd"));
		int r = userDao.login(f);
		if (r > 0) {// 当用户登录成功时
			HttpSession session = request.getSession();
			Music musico = musicDao.queryMusic(r);
			String music = musico != null ? musico.getMusicname() : "";
			int auto = musico != null ? musico.getAuto() : 1;
			session.setAttribute("userName", f.getUsername());// 保存用户名fg 
			session.setAttribute("uid", r);// 保存用户ID
			log.info(f.getUsername());
			log.info("" + r);
			request.setAttribute("returnValue", "success");// 保存提示信息
			log.info(music);
			session.setAttribute("music",music);
			session.setAttribute("auto", auto != 1 ? "" : "autoplay");
			request.getRequestDispatcher("DiaryServlet?action=listAllDiary").forward(request,
					response);// 重定向页面
		} else {// 当用户登录不成功时
	...
	public void checkUser(HttpServletRequest request,
		...
		String result = userDao.checkUser(username);		//调用UserDao类的checkUser()方法判断用户是否被注册
...
//		String result = userDao.save(sql);// 保存用户信息
		...
		if(userid > 0) {
			for (int i = 1; i <= 8; i ++) {
				Grid grid = gridDao.queryByGridId(i);
				GridUser gridUser = new GridUser();
				gridUser.setGrid(grid.getGrid());
				gridUser.setUserid(userDao.queryUserid(username));
				gridUser.setFirstline(grid.getFirstline());
				gridUser.setSecondline(grid.getSecondline());
				gridUser.setThirdline(grid.getThirdline());
				gridUser.setFourthline(grid.getFourthline());
				gridUserDao.save(gridUser);
			}
		}
	...
	private void resetPwd(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		...
		if (r > 0) {// 当用户登录成功时
			...
			if (res > 0) {// 提示问题答案错误
				out
						.println("<script>alert('密码修改成功！ ');history.back();</script>");
			} else {// 提示问题答案正确，返回密码
				out
						.println("<script>alert('出错了');window.location.href='UserServlet?action=resetPwd';</script>");
			}
		} else {// 当用户登录不成功时
			request.setAttribute("returnValue", "您输入的旧密码错误，请重新输入！");
			request.getRequestDispatcher("userMessage.jsp").forward(request,
					response);// 重定向页面
		}
	}
