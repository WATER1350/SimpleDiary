	...
	public List<Diary> queryDiary(String sql) {
		...
		try {// 捕获异常
			while (rs.next()) {
				...
				diary.setDetail(rs.getString(7));
				diary.setType(rs.getInt(8));
...
//		String sql = "INSERT INTO tb_diary (title,address,userid,detail,visibility) VALUES('"
//				+ diary.getTitle() + "','" + diary.getAddress() + "',"
//				+ diary.getUserid() + ",'" + diary.getDetail() + "',"
//				+ diary.getVisibility() + ")";		//保存数据的SQL语句
...
//		return ret;
		...
		int ret = 0;
		try {
		    ret = GetSqlSession.getSqlSession().insert(getNameSpace() + ".insertDiary",diary);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Reply> queryReply(int id) {
		List<Reply> ret = new ArrayList<Reply>();
		try {
		    ret = GetSqlSession.getSqlSession().selectList("reply" + ".queryReply",id);
			GetSqlSession.commit();
		} catch (Exception e) {
			e.printStackTrace();
			GetSqlSession.rollback();
